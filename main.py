...
def update_tree():
    global rootnode
    if rootnode==False:
        tk.messagebox.showerror("Error", "You have not set a root node!")
        return
...
def set_root():
    ...
    update_tree()
def figure():
    fig.savefig(str(date.today())+".png")

tk.Button(rtk, text="Set Root", command=set_root).grid(row=0, column=2, padx=10, pady=10)
tk.Button(rtk, text="Save Image", command=figure).grid (row=1, column=2, padx=10, pady=10)
tk.Button(rtk, text="Exit", command=kill).grid (row=2, column=2, padx=10, pady=10)
